...
import { X, Plus, Tag, Pin, Edit, Eye, Monitor } from "lucide-react"
...
interface CreateNoteModalWithDesktopProps {
  isOpen: boolean
  onClose: () => void
  userId: number
  desktops: Desktop[]
...
const CreateNoteModalWithDesktop = ({ isOpen, onClose, userId, desktops, onNoteCreated }: CreateNoteModalWithDesktopProps) => {
  const [title, setTitle] = useState("")
  const [content, setContent] = useState("")
  const [tags, setTags] = useState<string[]>([])
  const [newTag, setNewTag] = useState("")
  const [isPinned, setIsPinned] = useState(false)
  const [selectedDesktopId, setSelectedDesktopId] = useState<number>(0)
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setViewMode('edit')
      setError(null)
      // Set default desktop if available
      if (desktops.length > 0) {
        setSelectedDesktopId(desktops[0].id)
      } else {
        setSelectedDesktopId(0)
      }
    }
  }, [isOpen, desktops])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !content.trim() || selectedDesktopId === 0) {
      return
    }
    ...
    try {
      ...
      if (newNote) {
        ...
        setNewTag("")
        setIsPinned(false)
        setSelectedDesktopId(desktops.length > 0 ? desktops[0].id : 0)
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <CardDescription className="text-gray-600 mt-1">
                  Add a new note to your selected desktop
                </CardDescription>
              </div>
              <Button
                onClick={onClose}
          ...
          <CardContent className="text-left">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="desktop" className="text-sm font-medium text-gray-700 flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedDesktopId(Number(e.target.value))}
                  className="w-full px-3 py-2 border border-pink-300 rounded-md focus:ring-2 focus:ring-pink-500 focus:border-transparent bg-white"
                  required
                >
                  <option value={0}>Select a desktop...</option>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-pink-200">
                ...
                <Button
                  type="submit"
                  disabled={isLoading || !title.trim() || !content.trim() || selectedDesktopId === 0}
...
export default CreateNoteModalWithDesktop 